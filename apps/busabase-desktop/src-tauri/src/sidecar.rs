...
pub(crate) struct BusabaseSidecarStatus {
    running: bool,
    healthy: bool,
    port: u16,
    pid: Option<u32>,
...
#[tauri::command]
pub(crate) fn busabase_sidecar_status(app: AppHandle) -> Result<BusabaseSidecarStatus, String> {
    build_status(&app, None)
}

#[tauri::command]
pub(crate) fn start_busabase_sidecar(app: AppHandle) -> Result<BusabaseSidecarStatus, String> {
    {
...
#[tauri::command]
pub(crate) fn stop_busabase_sidecar(app: AppHandle) -> Result<BusabaseSidecarStatus, String> {
    stop_busabase_sidecar_process()?;
    build_status(&app, None)
}

pub(crate) fn stop_busabase_sidecar_process() -> Result<(), String> {
    let mut guard = sidecar_process()
        .lock()
        .map_err(|error| error.to_string())?;
