...
function recordMatchesFilter(record: RecordVO, filter: ViewFilterVO): boolean {
  const value = record.headCommit.payload[filter.fieldSlug];
...
export function applyViewConfig(records: RecordVO[], config?: ViewConfigVO | null): RecordVO[] {
  ...
  return [...filtered].sort((left, right) => {
    for (const sort of config.sorts) {
      const a = stringifyFieldValue(left.headCommit.payload[sort.fieldSlug]);
      const b = stringifyFieldValue(right.headCommit.payload[sort.fieldSlug]);
