...
export const fetchPendingSnapshot = async (
  ...
  try {
    ...
    return { count: counts.review, preview, capped: false };
  } catch (caught) {
    // `counts` takes no id, so it cannot answer NOT_FOUND for missing data — a
    // not-found here can only mean this server predates the route. Fall back to
    // counting the page, and SAY that it is capped so a caller can decline to
    // render a number it cannot stand behind.
