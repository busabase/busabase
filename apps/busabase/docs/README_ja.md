[English](../README.md) · [中文](./README_zh-CN.md) · **日本語** · [한국어](./README_ko.md)

# Busabase

> AIが生成したコンテンツ、ビジネスデータ、データセット、マルチモーダルナレッジのための、ローカルファーストなレビューデータベース。
...
```mermaid
flowchart LR
  A["人間またはAIエージェントがコンテンツ/データを提案"] --> B["変更リクエスト"]
  B --> C["オペレーション: 作成、更新、削除、またはバリアント"]
  C --> D["コミットスナップショット"]
  D --> E["レビュー"]
  E -->|エージェントに修正を依頼| J["ACPエージェントが提案を改善"]
  J --> C
  E -->|承認| F["マージ"]
  E -->|却下| G["却下されたリクエスト"]
  F --> H["信頼されたレコード"]
  H --> I["ダッシュボード、API、Next.js、ERPフロー、AIツール"]
  H --> K["自動化トリガー"]
  K --> L["Webhook、ワークフロー、または外部ACPエージェント"]
```
