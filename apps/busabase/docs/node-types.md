...
Nodepod and Local Node.js both let a running app reach `/api/v1` via **different mechanisms**: Nodepod's in-browser Service Worker passes the path through, while Local Node.js is reverse-proxied onto busabase's own origin so the path simply resolves. (srt can't serve a live preview at all, so this is moot for it.)
