...
test("Drive review upload stays in place and creates one multi-file Change Request", async ({
  ...
  // render this same string twice — once as a toast, once as the persistent task
  // status — so an unscoped match is a strict-mode violation for exactly as long
  // as the toast is still on screen. That is why this passes locally and fails
  // under CI load: the toast had already auto-dismissed locally. The panel row
  // is the durable one, and it is what "stays in place" actually means here.
  await expect(
    page.locator("[data-file-upload-task-panel]").getByText("Upload ready for review"),
  ).toBeVisible();
