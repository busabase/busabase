...
test("staged view controls recover a hidden conditioned field with one update CR", async ({
  ...
  await expect(page.getByTestId("view-control-sorts")).toContainText("1");

  await page.getByTestId("view-control-filters").click();
  const panel = page.getByTestId("view-controls-popover");
  ...
  await sortRow.getByRole("button").click();

  await panel.getByRole("button", { name: /Fields/ }).click();
  const hiddenFieldRow = panel.locator(`[data-view-field-slug="${hiddenField.slug}"]`);
  ...
  await expect(panel.getByRole("button", { name: "Discard staged changes" })).toBeVisible();
