...
/**
 * Strip a trailing slash run and a trailing `/api/v1` so an API base URL can be
 * passed straight through. Mirrors `normalizeBaseUrl` in busabase-sdk; kept
 * separate so the contract package stays dependency-free.
 */
...
export function nodeWebUrl({
  ...
  // `new URL` both validates and gives us the origin without any path the
  // caller may have appended.
