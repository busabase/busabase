...
都以**变更请求（change request）**的形式提出，带着说明、差异和作者。Agent 的任何
操作都不会悄悄生效，需要把关的随时可以停下来等人审。

## Agent 在这里如何工作

1. 先读懂相关的 Base、Doc 或 Skill。
2. 开一个变更请求，说明**改了什么**以及**为什么改**。
3. 我们团队的规矩：凡是碰到已发布内容的改动，都留给人审一遍。
4. 每一条事实性主张都要给出来源或内部记录 id。

## 为什么有些要审

工作区是一支由 Agent 和人共同编辑的共享记忆，因此一份可评审、可回滚的历史，
比单纯的写入速度更重要。改动要不要等人审，由凭证的权限级别决定；上面第 3 条是
我们团队自己挑出来的、值得慢一点的那部分。
`,
      changeRequest: {
        id: DOC_GUIDE_CR_ID,
...
Busabase 是 AI Agent 干活的工作区：每一次写入都是一个带说明、带差异、带历史的
变更请求，让一支由 Agent 和人组成的团队共享同一份可编辑的事实来源，而不会互相踩踏。

- Agent 直接写、需要把关的可以留给人审、历史可回滚。
