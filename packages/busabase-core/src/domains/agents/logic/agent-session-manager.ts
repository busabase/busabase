...
const REMOTE_SESSION_READY_TIMEOUT_MS = 30_000;
...
async function waitForRemoteSessionIdentity(
  ...
  const deadline = Date.now() + REMOTE_SESSION_READY_TIMEOUT_MS;
...
export async function promptAgentSession(
  ...
  try {
    ...
    const readiness = await waitForReadyOrCancellation(s.ready, promptController.signal);
