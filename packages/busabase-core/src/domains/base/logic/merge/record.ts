...
export const applyComputedRecordFields = async (
  ...
  args: {
    ...
    /** The record's own id — used by `formula`'s `RECORD_ID()`. On create,
     *  the id must be generated by the CALLER before this runs (compute
     *  happens before insert) — see `mergeRecordCreate`. */
    recordId?: string | null;
    /** ISO timestamp the record was originally created — used by
     *  `formula`'s `CREATED_TIME()`. Defaults to this write's own timestamp
     *  when omitted, which is only correct on create. */
  ...
  // supplies the real value. Computed BEFORE computeSystemFieldValues (not
  // after, as this used to be ordered) so a `formula` field can resolve its
  // sibling `{slug}` references from this same write via `values` below.
  ...
  const overrides = computeSystemFieldValues({
    ...
    recordCreatedAtIso: args.recordCreatedAtIso,
  });
...
export const mergeRecordCreate = async (ctx: MergeCtx, item: OperationPO, headCommit: CommitPO) => {
  ...
  // Generated BEFORE compute (not after, as this used to be ordered) so a
  // `formula` field's RECORD_ID() resolves on create, not just on update.
