 ...
 * actual navigations. Backs the Search dialog's "Recent" tab: an empty query
 ...
 *
 * A plain module-level singleton (not a React context/store) — every consumer
 * (the sidebar's `nodes.list` effect, the Recent tab, the shared `select()`
 * navigation helper) just calls `merge`/`markVisited`/`list`/`fuzzyMatch`
...
const fuzzyMatchForScope = (scope: string, query: string, limit = 20): KnownNode[] => {
  ...
  for (const node of listForScope(scope)) {
