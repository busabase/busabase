 ...
 * between the Busabase web app and Desktop shell, which embeds the local
 * sidecar in an iframe.
 *
 ...
 * the other.
 *
 * The scheme must match the OS registration in
...
/**
 * Parse the return URL emitted by the Cloud Connect callback.
 *
 * The mobile OAuth callback shares the `busabase://` scheme, so matching the
 * complete desktop host and path is required before accepting its status.
 */
export function parseDesktopCloudConnectReturnUrl(
  rawUrl: string,
  ...
  if (`${url.hostname}${normalizedPath}` !== DESKTOP_CLOUD_CONNECT_RETURN_PATH) return null;
