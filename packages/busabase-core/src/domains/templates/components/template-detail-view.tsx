    ...
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-6 p-6">
        ...
        <div className="flex flex-col items-end gap-1">
          ...
          {!canInstall ? (
            <span className="max-w-56 text-right text-[11px] text-muted-foreground">
              Space owners and admins only — a template can carry an app and a skill, which is code
              this space's agents will run.
            </span>
          ) : null}
        </div>
      </div>
      ...
      {template.agentPrompts.length > 0 ? (
        <section className="flex flex-col gap-2">
          ...
          <p className="text-muted-foreground text-xs">
            The agent can answer these because the template installs its author's manual alongside
            its tables — it does not have to guess your schema.
          </p>
        </section>
      ) : null}
      ...
      <section className="flex flex-col gap-2">
        ...
        <p className="text-muted-foreground text-xs">
          ...
          empty when you open it. The app's code and its manual are proposed as change requests for
          you to read first.
        </p>
      </section>
