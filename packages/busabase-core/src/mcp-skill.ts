 ...
 * tool (`BUSABASE_MCP_TARGET_SPACE_SCHEMA` via `additionalToolsInputSchema`). `auth_verify` omits
 * the argument on its first discovery call, then accepts it to resolve the selected Space's
 * bootstrap state. The self-hosted server has a single local workspace and no such argument —
 * telling its agent to pass `targetSpaceId` would teach a parameter that server rejects.
 *
