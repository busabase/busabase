...
export const PromptInputProvider = ({
  ...
  const add = useCallback((files: File[] | FileList) => {
    ...
    setAttachmentFiles((prev) => [
      ...prev,
      ...incoming.map((file) => {
        // Clipboard-pasted images get a generic name like "image.png" from the browser,
        // which causes overwrites when uploading to the same directory. Generate a unique
        // filename by appending a timestamp + random suffix.
        let filename = file.name;
        if (/^image\.\w+$/i.test(filename)) {
          const ext = filename.slice(filename.lastIndexOf("."));
          filename = `image-${Date.now()}-${nanoid(6)}${ext}`;
        }
        return {
          filename,
          id: nanoid(),
